...
features_location<-paste("UCI HAR Dataset", "/activity_labels.txt", sep = "")
...
col_name<-rbind(rbind(features_data,c(562,"Subject")),c(563,"activity_id"))[,2]
#col_name<-col_name[,2]
names(combined_data)<-col_name

#### 3. Filter mean and standard deviation ####
filtered_data<-combined_data[,grepl("mean|std|Subject|activity_id", names(combined_data))]

#### 4. Join and label data ####
...
## 4.2 Label data
# 4.2.1 Remove parentheses
names(filtered_data)<-gsub('\\(|\\)',"",names(filtered_data), perl = TRUE)
# 4.2.2 Make syntactically valid names
names(filtered_data)<-make.names(names(filtered_data))
# 4.2.3 Make clearer names
...
names(filtered_data) <- gsub('Gyro',"angular_speed",names(filtered_data))
names(filtered_data) <- gsub('Mag',"magnitude",names(filtered_data))
names(filtered_data) <- gsub('^t',"timeDomain.",names(filtered_data))
names(filtered_data) <- gsub('^f',"frequency_domain.",names(filtered_data))
